...
DATASET_LEVELS = [
    {"name": "build", "values": config["builds"]},
]
...
def write_subsample_config() -> None:
    ...
    for dataset in get_datasets(DATASET_LEVELS):
        out = build_subsample_config(subsample_config, dataset)

        build_name, = dataset

        path = f"results/{build_name}/subsample_config.yaml"
...
def write_refine_config() -> None:
    ...
    for dataset in get_datasets(DATASET_LEVELS):
        ...
        path = f"results/{build_name}/refine_config.yaml"
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w") as f:
            yaml.dump(out, f, sort_keys=False, Dumper=NoAliasDumper)
